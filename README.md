...
✅ AdGuard Home (unprivileged LXC) + DoH upstream
✅ HDD offload via LXC bind mounts (reduce SSD wear)
⏳ Wazuh SIEM + agents (next)
⏳ Tailscale remote access
⏳ Kali VM for controlled red teaming

🛡️ Secure HomeLab: Cybersecurity Range & Infrastructure
📌 Project Overview
...
The AdGuard LXC is unprivileged, so host-side ownership is mapped via Linux user namespaces:
container root → host UID/GID 100000
...
anomalies across Windows / Proxmox / AdGuard
...
🚀 Deployment Roadmap
 Deploy AdGuard Home in an unprivileged LXC
 Configure bind mounts for HDD storage optimization
